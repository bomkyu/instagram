...
/*document.addEventListener("click", function(event) {
    // 클릭된 요소가 left_wrap_event의 자식 요소이면 아무것도 하지 않음
    if (left_wrap_event.contains(event.target)) {
        return;
    }

    // 클릭된 요소가 left_wrap_event의 자식 요소가 아니면 모든 요소에서 active 클래스 제거
    left_menu_event.forEach(function(element, index){
        off(element);
    });

    left_2dep_event.forEach(function(element, index){
        off(element);
    });

    left_wrap_event.classList.remove('active');
});*/
